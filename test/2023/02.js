...
function a(contents) {
  return contents
    ...
    .map((x) => {
      const split = x.split(': ');
      const gameNumber = split[0].split(' ')[1];

      const sets = split[1].split('; ');
    
      const impossibleSet = sets.some((set) => {
        const colors = set.split(', ');

        return colors.some((color) => {
          if (color.includes('red') && parseInt(color.split(' ')[0], 10) > possibleRed) return true;
          if (color.includes('green') && parseInt(color.split(' ')[0], 10) > possibleGreen) return true;
          if (color.includes('blue') && parseInt(color.split(' ')[0], 10) > possibleBlue) return true;
        })
      });

      if (impossibleSet) return null;
...
function b(contents) {
  return contents
    ...
    .map((x) => {
      ...
      const highestSet = sets.reduce((acc, set) => {
        const red = set.match(/(\d+) red/);
        if (red && parseInt(red[1], 10) > acc.red) acc.red = parseInt(red[1], 10);

        const green = set.match(/(\d+) green/);
        if (green && parseInt(green[1], 10) > acc.green) acc.green = parseInt(green[1], 10);

        const blue = set.match(/(\d+) blue/);
        if (blue && parseInt(blue[1], 10) > acc.blue) acc.blue = parseInt(blue[1], 10);
...
describe.only('2023_02', () => {
  it('test A', () => {
    const result = a(`Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`);
  ...
  it('solve A', async () => {
    const filePath = new URL('./02.txt', import.meta.url);
    const contents = await readFile(filePath, { encoding: 'utf8' });

    const result = a(contents);

    assert.equal(result, 2593);
  });

  it('test B', () => {
    const result = b(`Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`);
