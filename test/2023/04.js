...
//  er, the small kind surrounded by water, not the floating kind. We really need to come up with 
...
// lists of numbers separated by a vertical bar (|): a list of winning numbers and then a list of numbers you have. 
...
// which of the numbers you have appear in the list of winning numbers. The first match makes the card
...
function a(contents) {
  return contents
    ...
    .map((card) => {
      ...
      const winningNumbers = sides[0].split(' ').filter((x) => x !== '');
      const yourNumbers = sides[1].split(' ').filter((x) => x !== '');
      
      const yourWinningNumbers = yourNumbers.filter((number) => winningNumbers.includes(number));

      return yourWinningNumbers.reduce((acc, _, index) => {
        if (index === 0) return 1;
        return acc * 2;
      }, 0)
    })
...
function b(contents) {
  ...
  const numberOfGames = cards.map(() => 1);
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    const result = yourWinningNumbers.length;

    for (let j = i + 1; j < i + result + 1 && j < cards.length; j++) {
      numberOfGames[j] += numberOfGames[i];
    }
  }
...
describe('2023_04', () => {
  it('test A', () => {
    const result = a(`Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19
Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1
Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83
Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11`);
  ...
  it('solve A', async () => {
    const filePath = new URL('./04.txt', import.meta.url);
    const contents = await readFile(filePath, { encoding: 'utf8' });

    const result = a(contents);

    assert.equal(result, 24848);
  });

  it('test B', () => {
    const result = b(`Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19
Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1
Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83
Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36
